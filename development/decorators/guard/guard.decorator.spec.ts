...
describe('Decorators: @Guard', () => {
    it('Should decorate findUser to have guard and to check if conditions', (done) => {
        ...
        const controller: { findUser(): { guards: Function[] } } = <any>Container.get(TestController);
        const currentGuard = Container.get<CanActivateResolver>(controller.findUser().guards[0]);
